...
import { synthesizeDebate } from "@/lib/debate-engine";

/**
 * Debate analysis endpoint.
 * GET /api/debate?pair=EURUSD&interval=1h&limit=300
 * Returns bull/bear debate + synthesis verdict.
 */
  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const smc = summarizeSMC(bars);
        const debate = synthesizeDebate(smc);

        return Response.json(
          { ...debate, pair, interval, barCount: bars.length },
